...
class KorrektlyClient {
  ...
  private async trackInstallation(): Promise<void> {
    ...
    if (!response.ok) {
      this.log(
        `Failed to install: ${response.status} | Status Text: ${response.statusText} | Body: ${response.body}`,
        "error"
      );
      return;
    }
    ...
    this.log(
      `Installation data sent successfully | Status: ${response.status} | Status Text: ${response.statusText}`,
      "info"
    );
    return;
  }
